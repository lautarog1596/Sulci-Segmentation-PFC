...
def compute_per_channel_dice(input, target, epsilon=1e-6, weight=None):
    ...
    # input = F.softmax(input, dim=1) ######## NUEVO NUEVO NUEVO NUEVO no va xq ya lo tiene en AbstractDice

    input = flatten(input) # se pasa a float por si se entrena con half precision
    target = flatten(target) # (N, C, D, H, W) -> (C, N * D * H * W)
    target = target.float()
    ...
    # can sum across the last array dimension by using sum(-1)
    # here we can use standard dice (input + target).sum(-1) or extension (see V-Net) (input^2 + target^2).sum(-1)
    denominator = (input * input).sum(-1) + (target * target).sum(-1)
    return 2 * (intersect / denominator.clamp(min=epsilon)) # clamp a un minimo para no dividir por cero
...
class GeneralizedDiceLoss(_AbstractDiceLoss):
    ...
    def dice(self, input, target, weight):
        ...
        input = flatten(input) # (N, C, D, H, W) -> (C, N * D * H * W)
        target = flatten(target)
        target = target.float() # se pasa a float por si se entrena con half presicion
        ...
        w_l = target.sum(-1) # sum(-1) indica sumar en el último canal
