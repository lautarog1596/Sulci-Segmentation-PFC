...
def get_logger(name, level=logging.INFO):
    ...
    if loggers.get(name) is not None:
        ...
        # Logging to file
        cwd_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'loggers')
        if not os.path.exists(cwd_path):
            os.mkdir(cwd_path)

        filename = os.path.join(cwd_path, name + '.log')
...
################## Las 3 siguientes func no se usan ##################
...
def expand_as_one_hot(input, C, ignore_index=None):
    ...
    if ignore_index is not None:
        ...
        return torch.zeros(shape).to(input.device).scatter_(1, input, 1)
            # scatter arg1: dimension en la cual se va a hacer scatter, en este caso: dim 1 de Nx1xDxHxW
            # arg2: indices en los planos en los cuales se hará el scatter. Ej. numero 7, se hará scater en Nx[7]xDxHxW
            # arg3: valor que se scatteará, en este caso el 1 para one hot
