...
def collect_and_download(out_dir, subject, params, reg):
    ...
    prefix =  f'HCP_1200/{subject}/{reg}'
    s3_keys = bucket.objects.filter(Prefix=prefix)

    # me quedo solo con las que me interesan
    s3_keylist= [key.key for key in s3_keys
                    if (
                        fnmatch.fnmatch(key.key, '*/T1w_acpc_dc_restore_brain.nii.gz') ######################## poner los archivos que quiero descargar!!!!!!!
...
def main():
    ...
    path_subjcts = '/media/lau/datos_facultad/pfc/Datos/Val'
    ...
    for i, id_paciente in enumerate(list_id_pacientes, 1):
        ...
        collect_and_download(out_dir=path_subjcts, subject=id_paciente, params=params, reg=reg)
